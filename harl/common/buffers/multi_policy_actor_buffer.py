"""On-policy buffer for multi-policy actor with multi-head support."""
...
class MultiPolicyActorBuffer:
    """On-policy buffer for multi-policy actor data storage.
    ...
    def insert(
        ...
        stability_VSS = stability_VSS.reshape(-1, 1)
        self.stability_VSS[self.step] = stability_VSS.copy()
    ...
    def get_reward_components(self):
        """Get all reward components in a structured format.
        Returns:
            dict: organized reward components
        """
        reward_components = {}

        # 主要奖励组件
        if 'safety' in [self.critic_head_names.get(i) for i in range(self.critic_head_num)]:
            reward_components['safety'] = self.rewards_safety
        if 'efficiency' in [self.critic_head_names.get(i) for i in range(self.critic_head_num)]:
            reward_components['efficiency'] = self.rewards_efficiency
        if 'stability' in [self.critic_head_names.get(i) for i in range(self.critic_head_num)]:
            reward_components['stability'] = self.rewards_stability
        if 'comfort' in [self.critic_head_names.get(i) for i in range(self.critic_head_num)]:
            reward_components['comfort'] = self.rewards_comfort

        return reward_components

    def get_reward_components_array(self):
        """Get reward components as a structured array for multi-head critic.
        Returns:
            np.ndarray: shape (episode_length, n_rollout_threads, critic_head_num)
        """
        # 创建形状为(episode_length, n_rollout_threads, critic_head_num)的数组
        reward_components_array = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.critic_head_num),
            dtype=np.float32
        )

        # 根据critic_head_names填充对应的奖励组件
        for i in range(self.critic_head_num):
            head_name = self.critic_head_names.get(i, f"head_{i}")

            if head_name == "safety":
                reward_components_array[:, :, i] = self.rewards_safety[:, :, 0]
            elif head_name == "efficiency":
                reward_components_array[:, :, i] = self.rewards_efficiency[:, :, 0]
            elif head_name == "stability":
                reward_components_array[:, :, i] = self.rewards_stability[:, :, 0]
            elif head_name == "comfort":
                reward_components_array[:, :, i] = self.rewards_comfort[:, :, 0]
            else:
                # 如果有未识别的头名称，使用零奖励
                reward_components_array[:, :, i] = 0.0

        return reward_components_array
